...
local function gradCheckCrit2()
  ...
  opt.batch_size = 6
  local lm = nn.LanguageModel(opt)
  local crit = nn.LanguageRankingCriterion()
  lm:type(dtype)
  crit:type(dtype)
  ...
  seq[{ {4, 7}, 1 }] = 0
  seq[{ {5, 7}, 4 }] = 0
  local Labels = torch.cat(seq, seq)  -- [seq | seq]
  local imgs = torch.randn(opt.batch_size*2, opt.input_encoding_size):type(dtype)

  -- evaluate the analytic gradient
  local output = lm:forward{imgs, Labels}
  local loss = crit:forward(output, seq)
  local gradOutput = crit:backward(output, seq)
  local gradInput, dummy = unpack(lm:backward({imgs, Labels}, gradOutput))

  -- create a loss function wrapper
  local function f(x)
    local output = lm:forward{x, Labels}
    local loss = crit:forward(output, seq)
    return loss
  end
...
local function overfit2()
  ...
  opt.batch_size = 6
  local lm = nn.LanguageModel(opt)
  local crit = nn.LanguageRankingCriterion(1)
  lm:type(dtype)
  crit:type(dtype)
  ...
  local function lossFun()
    ...
    local loss = crit:forward(output, seq)
    local gradOutput = crit:backward(output, seq)
    lm:backward({imgs, Labels}, gradOutput)
    return loss
  end
...
tests.gradCheckCrit1 = gradCheckCrit1
tests.gradCheckCrit2 = gradCheckCrit2
tests.gradCheckLM = gradCheckLM
tests.overfit1 = overfit1
tests.overfit2 = overfit2
tests.sample = sample

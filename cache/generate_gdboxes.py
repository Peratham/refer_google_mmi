...
It provides candidate bboxes to evaluate "sent --> bbox"
1) we first feed each bbox feature (global+referr+location) to the model.
2) Then we rank the output loss of each bbox, and pick up the lowest one. 
...
for image_id in image_ids:
	...
	for ann in anns:
		imageToBoxes[image_id] = imageToBoxes.get(image_id, []) + [ann['bbox']]
print 'image_id --> boxes computed.'

# save
file_folder = './box/'+dataset_splitBy

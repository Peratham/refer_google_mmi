...
-------------------------------------------------------------------------------
-- Language Model-Ranking Criterion
-------------------------------------------------------------------------------

local crit, parent = torch.class('nn.LanguageRankingCriterion', 'nn.Criterion')
function crit:__init(margin)
  parent.__init(self)
  self.margin = margin or 1  -- margin value
end
--[[
input is Tensor of size (D+2)x(N*2)x(M+1) and Tensor of size DxN
...
seq is ground-truth labels of size DxN
output is the ranking loss.
--]]
function crit:updateOutput(input, seq)
  self.gradInput:resizeAs(input):zero() -- reset to zeros
  local L, NpN, Mp1 = input:size(1), input:size(2), input:size(3)
  local D, N = seq:size(1), seq:size(2)
  assert(D == L-2, 'input Tensor should be 2 larger in time')
  assert(N*2 == NpN, 'batch_size of input should be 2 times as seq')

  -- compute logprobs of each sent
  local sentLogProbs = torch.zeros(2, N)  -- 1st half are positive pairs, and 2nd half are negative pairs

  for b = 1, N do
    local first_time = true
    local n = 0  -- count words
    for t=2, L do
      -- fetch the index of the next token in the sequence
      local target_index
      if t-1 > D then
      ...
      if target_index ~= 0 then
        n = n + 1
        -- b-th positive pair
        sentLogProbs[1][b] = sentLogProbs[1][b] + input[{ t, b, target_index }]
        self.gradInput[{t, b, target_index}] = 1
        -- b-th negative pair
        sentLogProbs[2][b] = sentLogProbs[2][b] + input[{ t, N+b, target_index }]
        self.gradInput[{t, N+b, target_index}] = 1
      end
    end
    sentLogProbs[1][b] = sentLogProbs[1][b]/n
    self.gradInput[{ {}, b, {} }]:div(n)

    sentLogProbs[2][b] = sentLogProbs[2][b]/n
    self.gradInput[{ {}, N+b, {} }]:div(n)
  end

  -- compute Ranking loss for each pair of (nagative logprob over positive logprob)
...
function crit:updateGradInput(input, seq)
  ...
  for b=1, N do
    ...
    if dsentLogProbs[1][b]~=0 then
      self.gradInput[{ {}, b, {} }]:mul(dsentLogProbs[1][b])
    else
      self.gradInput[{ {}, b, {} }]:zero()
    end
    -- backprop on gradInput (negative half)
    if dsentLogProbs[2][b]~=0 then
      self.gradInput[{ {}, N+b, {} }]:mul(dsentLogProbs[2][b])
    else
      self.gradInput[{ {}, N+b, {} }]:zero()
    end
  end

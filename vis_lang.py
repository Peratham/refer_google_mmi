...
"""
1) load valtest/dataset/model_idX_test_refToEval.json, which is {ref_id: {'sent', 'Bleu1', 'Bleu2', 'Bleu_3', 'Bleu_4', 'METEOR', 'ROUGE_L', 'CIDER'}}
2) load refer
3) write html and save to vis/
Note, the dataset here means "dataset_splitBy" or "dataset_splitBy"
"""
...
evaluation = json.load(open(osp.join('cache/lang', dataset_splitBy, 'model_id'+str(model_id)+'_'+split+'_out.json'), 'r'))
...
# write html
vis_folder = osp.join('cache/vis', dataset_splitBy)
if not osp.isdir(vis_folder):
  os.mkdir(vis_folder)
html_path = osp.join('cache/vis', dataset_splitBy, 'model_id'+str(model_id)+'_'+split)
...
num_per_page = 500
for page_id, s in enumerate(range(0,len(ref_ids), num_per_page)):
  ...
  cnt = 0
  for j in range(s, min(s+num_per_page, len(ref_ids))):
    ref_id = ref_ids[j]
    image_id = refToImageId[ref_id]
...
html.write('<ul>')
for page_id, s in enumerate(range(0,len(ref_ids), num_per_page)):
  page_html = 'model_id'+str(model_id)+'_'+split+'_'+str(page_id)+'.html'
  print page_html
